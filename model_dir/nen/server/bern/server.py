...
import json
import numpy as np
from biobert_ner.run_ner import FLAGS
...
class GetHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "type" not in data:
            err_msg = [{"error": "no type"}]
            print(datetime.now().strftime(time_format),
                  '[' + cur_thread_name + ']', err_msg)
            self.wfile.write(json.dumps(err_msg).encode('utf-8'))
            return

        mention_type = str(data["type"])

        # NER
        result_dict = self.tag_entities(
            text, mention_type, cur_thread_name, is_raw_text=True, reuse=False)
    ...
    def tag_entities(self, text, mention_type, cur_thread_name, is_raw_text, reuse=False):
        # print("------text in tag_entities-----")
        # print(text)
        ...
        if mention_type == "mutation":
            ...
            num_entities = len(dict_list[0]["entities"]["mutation"])

        else:
            dict_list = [{
                "pmid": f"{text_hash}-{cur_thread_name}",
                "mutation_model": "tmVar 2.0",
                "entities": {"mutation": []},
                "abstract": text,
            }]

            for mt in ["drug", "gene", "disease", "species"]:
                if mt == mention_type:
                    dict_list[0]["entities"][mt] = [{"start": 0, "end": len(text) - 1}]
                else:
                    dict_list[0]["entities"][mt] = []

            num_entities = 1

        tagged_docs = dict_list
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--n_pmid_limit', type=int,
                           help='max # of pmids', default=10000)
