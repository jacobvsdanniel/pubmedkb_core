...
def convert_input_file_to_tensor_dataset(
    ...
    with open(pred_config.input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            e11_p = [i for i, t in enumerate(tokens) if t == "<e1>"]
            e12_p = [i for i, t in enumerate(tokens) if t == "</e1>"]
            assert len(e11_p) == len(e12_p)
            e21_p = [i for i, t in enumerate(tokens) if t == "<e2>"]
            e22_p = [i for i, t in enumerate(tokens) if t == "</e2>"]
            assert len(e21_p) == len(e22_p)
            ...
            for l, r in zip(e11_p, e12_p):
                for i in range(l + 1, r + 2):
                    e1_mask[i] = 1
            for l, r in zip(e21_p, e22_p):
                for i in range(l + 1, r + 2):
                    e2_mask[i] = 1
...
def predict(pred_config):
    ...
    preds.dump(pred_config.output_file + ".npy")
