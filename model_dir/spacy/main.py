...
def get_merged_hypenated_verb_sentence_list(sentence_list, verb_set):
    def get_merged_hyphenated_verb(match):
        ...
        if len(term) >= 7 and term in verb_set:
            return term
...
def add_spacy_data(data, start, end, verb_set):
    ...
    sentence_list = get_merged_hypenated_verb_sentence_list(sentence_list, verb_set)
...
def run_spacy_relation_extraction(arg):
    ...
    verb_set = set(verb for verb, _count in verb_data[1:])

    for start in range(0, sentences, arg.batch_size):
        end = min(start + arg.batch_size, sentences)
        add_spacy_data(data, start, end, verb_set)
